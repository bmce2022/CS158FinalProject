package ml.classifiers;

import ml.data.CrossValidationSet;
import ml.data.DataSet;
import ml.data.DataSetSplit;
import ml.data.Example;

/**
 * Experiments for Assignment 8, Question 4
 * Grid search over hidden nodes and learning rate (eta)
 * @author Bleecker Coyne
 */
public class ExperimentsP4 {
    
    private static final String TITANIC_TRAIN = "/Users/bleeckercoyne/Desktop/CS 158/assign-8-bmcoyne/data/titanic-train.csv";
    private static final int ITERATIONS = 200;
    private static final int NUM_FOLDS = 10;
    private static final int[] HIDDEN_NODES = {1, 2, 3, 4, 5};
    private static final double[] LEARNING_RATES = {0.01, 0.05, 0.1, 0.15, 0.2};

    public static void main(String[] args) {
        DataSet fullData = new DataSet(TITANIC_TRAIN, DataSet.CSVFILE);
        CrossValidationSet cvSet = fullData.getRandomCrossValidationSet(NUM_FOLDS);
        
        // Print header row
        System.out.println("Hidden nodes\tLearning rate\tTraining average\tTesting average");

        double bestTestAccuracy = 0.0;
        int bestHiddenNodes = 0;
        double bestLearningRate = 0.0;
        double bestTrainAccuracy = 0.0;
        
        // Grid search: test all combinations
        for (int hiddenNodes : HIDDEN_NODES) {
            for (double eta : LEARNING_RATES) {
                double totalTrainAccuracy = 0.0;
                double totalTestAccuracy = 0.0;
                
                // Run 10-fold cross validation for this combination
                for (int fold = 0; fold < NUM_FOLDS; fold++) {
                    DataSetSplit split = cvSet.getValidationSet(fold);
                    DataSet trainData = split.getTrain();
                    DataSet testData = split.getTest();

                    TwoLayerNN nn = new TwoLayerNN(hiddenNodes);
                    nn.setEta(eta);
                    nn.setIterations(ITERATIONS);
                    nn.train(trainData);
                    
                    double trainAccuracy = calculateAccuracy(nn, trainData);
                    totalTrainAccuracy += trainAccuracy;
                    double testAccuracy = calculateAccuracy(nn, testData);
                    totalTestAccuracy += testAccuracy;
                }
                
                // Calculate averages across all folds
                double avgTrainAccuracy = totalTrainAccuracy / NUM_FOLDS;
                double avgTestAccuracy = totalTestAccuracy / NUM_FOLDS;
                
                // Print results for this combination
                System.out.printf("%d\t%.2f\t%.6f\t%.6f%n", 
                    hiddenNodes, eta, avgTrainAccuracy, avgTestAccuracy);
                
                // Track best model (based on test accuracy)
                if (avgTestAccuracy > bestTestAccuracy) {
                    bestTestAccuracy = avgTestAccuracy;
                    bestHiddenNodes = hiddenNodes;
                    bestLearningRate = eta;
                    bestTrainAccuracy = avgTrainAccuracy;
                }
            }
        }
        
        // Print best model summary
        System.out.println("\n========================================");
        System.out.println("BEST MODEL:");
        System.out.println("========================================");
        System.out.printf("Hidden nodes: %d%n", bestHiddenNodes);
        System.out.printf("Learning rate (eta): %.2f%n", bestLearningRate);
        System.out.printf("Iterations: %d%n", ITERATIONS);
        System.out.printf("10-fold training accuracy: %.6f (%.2f%%)%n", 
            bestTrainAccuracy, bestTrainAccuracy * 100);
        System.out.printf("10-fold testing accuracy: %.6f (%.2f%%)%n", 
            bestTestAccuracy, bestTestAccuracy * 100);
        System.out.println("========================================");
    }
    
    /**
     * Calculate accuracy on a dataset
     * @param nn The neural network
     * @param data The dataset
     * @return The accuracy as a double between 0 and 1
     */
    private static double calculateAccuracy(TwoLayerNN nn, DataSet data) {
        int correct = 0;
        int total = 0;
        
        for (Example example : data.getData()) {
            double prediction = nn.classify(example);
            double label = example.getLabel();
            
            if (prediction == label) {
                correct++;
            }
            total++;
        }
        
        return (double) correct / total;
    }
}